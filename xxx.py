...
class Test(object):
	...
	def test_tail(self,data):
		...
		return data
test = Test()
def worker_func(in_queue:JoinableQueue, out_queue:Queue ):
	...
	while True:
		...
		if dat==None:
			...
			break
		num= dat 
		out_queue.put((test.test_head(num), test.test_tail(num)))
		in_queue.task_done()
...
tf.flags.DEFINE_integer('num_worker', 3 , 'worker for test')
...
class Test(object):
	...
	def __init__(self, test_data_size, loader=None):
		...
		for item in loader.train_data_list:

			self.all_data_dict[item] = len(self.all_data_dict)
		for item in loader.valid_data_list:

			self.all_data_dict[item] = len(self.all_data_dict)

		for item in loader.test_data_list:
	...
	def test_head(self, predict_h, temp_h, triple):
		...
		if f_rank < 10:
			l_filter_hit_10 +=1
		return r_rank, f_rank, l_raw_hit_10, l_filter_hit_10
	...
	def test_tail(self, predict_t, temp_t, triple):
		...
		return r_rank, f_rank, r_raw_hit_10, r_filter_hit_10
...
def worker_func(in_queue:JoinableQueue, out_queue:Queue, test:Test, ):
	...
	while True:
		...
		if dat==None:
			in_queue.task_done()
			break
		predict_h, temp_h, predict_t,temp_t, item = dat 
		print(item)
		out_queue.put((test.test_head(predict_h,temp_h,item), test.test_tail(predict_t, temp_t, item)))
		in_queue.task_done()
...
def test_model():
	...
	test = Test(test_data_size=len(data_loader.test_data_list), loader=data_loader)
	evaluation_queue=JoinableQueue()
	result_queue=Queue()
	for i in range(flags.num_worker):
		worker=Process(target=worker_func,args=(evaluation_queue,result_queue,test))
		worker.start()
		print("work %d start!"% i)
	...
	with tf.Graph().as_default():
		with tf.Session() as sess:
			...
			for data in data_loader.get_predict_instance():
				...
				evaluation_queue.put((predict_h[0], temp_h, predict_t[0], temp_t, item))
				evaluation_count+=1
			...
			print("l_raw_mean_rank {:.3f} l_filter_mean_rank {:.3f} l_raw_hit_10 {:.5f} l_filter_hit_10 {:.5f}".format(l_raw_mean_rank/index, l_filter_mean_rank/index,
				l_raw_hit_10/index,l_filter_hit_10/index))

			print("r_raw_mean_rank {:.3f} r_filter_mean_rank {:.3f} r_raw_hit_10 {:.5f} r_filter_hit_10 {:.5f}".format(r_raw_mean_rank/index, r_filter_mean_rank/index,
