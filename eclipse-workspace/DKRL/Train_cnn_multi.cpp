...
void run(int n_in,int n_1_in,int n_w_in,double rate_in,double margin_in,int method_in)
{
	...
	stringstream ss;
	ss << n_w;
	string n_w_string="./data/word_vec.txt";
	...
	while (fscanf(f1,"%s",buf)==1)
	{
		...
		fscanf(f1,"%lf",&word_vec[iii][0]);
		for (int ii=1; ii<n_w; ii++)
			fscanf(f1,"%lf",&word_vec[iii][ii]);
		norm(word_vec[iii]);
		iii++;
	}
	//convolution init
...
void prepare(){
    FILE* f1 = fopen("../data/entity_word/entity2id.txt","r");
	FILE* f2 = fopen("../data/entity_word/relation2id.txt","r");
	FILE* f3 = fopen("../data/entity_word/word2id.txt","r");
	FILE* f4 = fopen("../data/entity_word/entityWords.txt","r");
	int x;
